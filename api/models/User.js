    ...
    generateAllPathTenure: function (data, cashflow, callback) {
        ...
        Grid.findGridByType(data, function (res, err) {
            _.each(res, function (n) {
                ...
                } else if (i < totalpath && tenure < cashflow.length) {
                    pathtemp[i] = 0;
                }
                if (i === (totalpath - 1) && tenure < cashflow.length) {
                    pathtemp = _.sortBy(pathtemp, function (n) {
                        return n;
                    });
                    pathvalgrid.push(pathtemp);
                    pathtemp = [];
                }
            });
            var tenure=[];
            _.each(pathvalgrid, function (key) {
                tenure.push({
                    median1:key[Math.ceil((totalpath-1)/100)],
                    median50:key[Math.ceil((totalpath-1)/50)],
                    median99:key[Math.ceil(99*((totalpath-1)/100))]
                });
            })
